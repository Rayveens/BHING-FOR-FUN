...
void AutoTele::OnLoad() {
	...
	settingsTab = new UITab("Varios", BH::settingsUI);
	unsigned int Y = 12;
	unsigned int col = 40;

	new Texthook(settingsTab, col+20, Y, "Activables");

	new Checkhook(settingsTab, col, (Y += 15),
			&Toggles["Draw Destination"].state, "Trazar Caminos");

	new Checkhook(settingsTab, col, (Y += 15),
			&Toggles["Quest Drop Warning"].state, "Aviso de Quest Items");

	new Texthook(settingsTab, col+20, (Y += 22), "Creación de Partidas");
	new Checkhook(settingsTab, col, (Y += 15),
			&(*bnetBools)["Autofill Last Game"], "Completar Ultima Partida");
	new Checkhook(settingsTab, col, (Y += 15),
			&(*bnetBools)["Autofill Next Game"], "Completar Siguiente Partida");
	new Checkhook(settingsTab, col, (Y += 15),
			&(*bnetBools)["Autofill Last Password"], "Completar Ultima Contraseña");
	new Checkhook(settingsTab, col, (Y += 15),
			&(*bnetBools)["Autofill Description"], "Completar Descripción");
	new Checkhook(settingsTab, col, (Y += 15),
			&(*gamefilterBools)["Show Difficulty"], "Mostrar Dificultad");
	new Checkhook(settingsTab, col, (Y += 15),
			&(*gamefilterBools)["Show Gameserver"], "Mostrar Servidor");
	...
	new Texthook(settingsTab, 250, 12, "Colores");

	new Colorhook(settingsTab, 250, 27, &Colors[0], "Ruta");

	new Colorhook(settingsTab, 250, 42, &Colors[1], "Siguiente");

	new Colorhook(settingsTab, 250, 57, &Colors[2], "Otros");

	new Colorhook(settingsTab, 250, 72, &Colors[3], "Portal");

	new Colorhook(settingsTab, 250, 87, &Colors[4], "Previa");

	new Colorhook(settingsTab, 250, 102, &Colors[5], "Otros Extras");
}
