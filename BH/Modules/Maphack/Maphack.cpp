...
void Maphack::ResetPatches() {	
	...
	if (Toggles["Corregir CPU"].state)
		cpuPatch->Install();
	else
		cpuPatch->Remove();

	if (Toggles["Incrementar FPS"].state)
		fpsPatch->Install();
	else
		fpsPatch->Remove();	
}
...
void Maphack::OnLoad() {
	...
	new Texthook(settingsTab, 80, 3, "Activables");
	unsigned int Y = 0;
	int keyhook_x = 150;
	int col2_x = 250;

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Show Monsters"].state, "Ver Monstruos");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Show Monsters"].toggle, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Monster Enchantments"].state, "Ver Mejoras");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Monster Enchantments"].toggle, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Monster Resistances"].state, "Ver Resistencias");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Monster Resistances"].toggle, "");
	
	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Show Missiles"].state, "Ver Misiles");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Show Missiles"].toggle, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Show Chests"].state, "Ver Cofres");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Show Chests"].toggle, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Display Level Names"].state, "Nombre de Zonas");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Display Level Names"].toggle, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Auto Reveal"].state, "Auto Revelar");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Auto Reveal"].toggle, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Force Light Radius"].state, "Radio de Luz");
	new Keyhook(settingsTab, keyhook_x, (Y + 2), &Toggles["Force Light Radius"].toggle, "");

	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Remove Weather"].state, "Eliminar Clima");
	...
	new Checkhook(settingsTab, 4, (Y += 15), &Toggles["Remove Shake"].state, "Remover Temblor");
	...
	new Texthook(settingsTab, col2_x + 5, 4, "Color de Misiles");
	...
	new Texthook(settingsTab, col2_x + 5, 80, "Color de Monstruos");
	new Colorhook(settingsTab, col2_x, 95,  &monsterColors["Normal"], "Normal");
	new Colorhook(settingsTab, col2_x, 110, &monsterColors["Minion"], "Minion");
	new Colorhook(settingsTab, col2_x, 125, &monsterColors["Champion"], "Champion");
	new Colorhook(settingsTab, col2_x, 140, &monsterColors["Boss"], "Boss");

	new Texthook(settingsTab, 41, (Y += 28), "Revelar:");

	vector<string> options;
	options.push_back("Todo");
	options.push_back("Acto");
	options.push_back("Zona");
