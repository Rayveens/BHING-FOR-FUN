...
void Item::DrawSettings() {
	settingsTab = new UITab("Items", BH::settingsUI);
	int y = 10;
	int keyhook_x = 230;

	new Checkhook(settingsTab, 4, y, &Toggles["Show Ethereal"].state, "Mostrar Ethereo");
	new Keyhook(settingsTab, keyhook_x, y+2, &Toggles["Show Ethereal"].toggle, "");
	y += 15;

	new Checkhook(settingsTab, 4, y, &Toggles["Show Sockets"].state, "Mostrar Sockets");
	new Keyhook(settingsTab, keyhook_x, y+2, &Toggles["Show Sockets"].toggle, "");
	y += 15;

	new Checkhook(settingsTab, 4, y, &Toggles["Show iLvl"].state, "Mostrar iLvl");
	...
	new Checkhook(settingsTab, 4, y, &Toggles["Shorten Item Names"].state, "Resumir Nombres");
	new Keyhook(settingsTab, keyhook_x, y+2, &Toggles["Shorten Item Names"].toggle, "");
	y += 15;

	new Checkhook(settingsTab, 4, y, &Toggles["Always Show Items"].state, "Mostrar Items Siempre");
	new Keyhook(settingsTab, keyhook_x, y + 2, &Toggles["Always Show Items"].toggle, "");
	y += 15;

	new Checkhook(settingsTab, 4, y, &Toggles["Always Show Item Stat Ranges"].state, "Mostrar Variaciones");
	new Keyhook(settingsTab, keyhook_x, y+2, &Toggles["Always Show Item Stat Ranges"].toggle, "");
	y += 15;
	
	new Checkhook(settingsTab, 4, y, &Toggles["Advanced Item Display"].state, "Información Avanzada");
	new Keyhook(settingsTab, keyhook_x, y+2, &Toggles["Advanced Item Display"].toggle, "");
	y += 15;

	new Checkhook(settingsTab, 4, y, &Toggles["Item Drop Notifications"].state, "Notificaciones de Drop");
	...
	new Checkhook(settingsTab, 4, y, &Toggles["Verbose Notifications"].state, "Notificacines Verbales");
	...
	new Keyhook(settingsTab, 4, y+2, &showPlayer, "Mostrar Inventario Aliado:   ");
	y += 15;

	new Texthook(settingsTab, 4, y, "Filtrado:");
	
	vector<string> options;
	options.push_back("0 - Ninguno");
	options.push_back("1 - Minimo");
	options.push_back("2 - Moderado");
	options.push_back("3 - Ultra");
	new Combohook(settingsTab, 85, y, 120, &filterLevelSetting, options);

	new Texthook(settingsTab, 234, y, "Niveles de Ping <=:");
...
void __stdcall Item::OnProperties(wchar_t * wTxt)
{
	...
	static wchar_t wDesc[512];// a buffer for converting the description
	...
	if (Toggles["Advanced Item Display"].state) {
		...
		if (desc != "") {
			auto chars_written = MultiByteToWideChar(CODE_PAGE, MB_PRECOMPOSED, desc.c_str(), -1, wDesc, 512);
