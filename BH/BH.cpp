...
void BH::Initialize()
{
	...
	config = new Config("BSMH_settings.cfg");
	if(!config->Parse()) {
		config->SetConfigName("BSMH_Default.cfg");
		if(!config->Parse()) {
			string msg = "No se encuentra BSMH settings.\nAttempted to load " +
				path + "BSMH_settings.cfg (fallo).\nAttempted to load "+
				path + "BSMH_Default.cfg (fallo).\n\nDefaults loaded.";
			MessageBox(NULL, msg.c_str(), "Fallo al cargar BSMH config", MB_OK);
		}
	}
	itemConfig = new Config("BSMH.cfg");
	itemConfig->Parse();
...
bool BH::ReloadConfig() {
	if (initialized){
		if (D2CLIENT_GetPlayerUnit()) {
			PrintText(0, "Leyendo config: %s", config->GetConfigName().c_str());
			PrintText(0, "Recargando Item config: %s", itemConfig->GetConfigName().c_str());
		}
		config->Parse();
		itemConfig->Parse();
